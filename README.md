...
1. **Tab Navigation:** Changes the `ctrl+tab` and `ctrl+shift+tab` key bindings to cycle through the tabs in the **active pane** (note that this is unlike the built in behavior for `cmd+shift+]` and `cmd+shift+[`, which cycle through the tabs in the active window, across all split panes) in the order that they appear in the pane (as opposed to in order of recency, as is the default).
